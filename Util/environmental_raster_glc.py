...
raster_metadata = {
    'alti': {'nan': 0},
    'awc_top': {'nan': -1, 'one_hot': False, 'encode':True},
    'bs_top': {'nan': -1, 'one_hot': False, 'encode':True},
    'cec_top': {'nan': -1, 'one_hot': False, 'encode':True},
    ...
    'clc': {'attrib_column': 'CLC_CODE', 'one_hot': False, 'nan': -1, 'encode':True},
    'crusting': {'nan': -1., 'one_hot': False, 'encode':True},
    'dgh': {'nan': -1, 'one_hot': False, 'encode':True},
    'dimp': {'nan': -1, 'one_hot': False, 'encode':True},
    'erodi': {'nan': -1, 'one_hot': False, 'encode':True},
    'etp': {'nan': 132, 'one_hot': False},
    'oc_top': {'nan': -1, 'one_hot': False, 'encode':True},
    'pd_top': {'nan': -1, 'one_hot': False, 'encode':True},
    'proxi_eau_fast': {'attrib_column': None, 'nan': -1, 'dtype': rasterio.ubyte},
    'text': {'nan': -1, 'one_hot': False, 'encode':True},
}
...
class Raster(object):
    ...
    def __init__(self, path, nan=None, normalized=False, transform=None, size=64, one_hot=False,
                 attrib_column='QUANTI', max_val=255, min_val=0, dtype=None, encode=False):
        """
    ...
    def _get_patch(self, item, cancel_one_hot=False):
        ...
        if self.size == 1:
            ...
            if self.encode:
                try:
                    med=int(stats.mode(patch[patch!=self.no_data],axis=None)[0][0])  ###replace no_data by median value
                except IndexError:
                    print("Index exception at item ",str(item)," var= ",self.name)
                    med=0
                    pass
                patch[patch==self.no_data]=med
                for i in self.unique_values: patch[patch==i]=i
        ...
        # environmental tensor
            ...
            patch = self.raster[row_num-half_size:row_num+half_size,
                    col_num - half_size:col_num+half_size].astype(np.float32)
            
            if self.encode:
                ### replace each unique value by its code
                try:
                    med=int(stats.mode(patch[patch!=self.no_data],axis=None)[0][0])  ###replace no_data by median value
                except IndexError:
                    print("Index exception at item ",str(item)," var= ",self.name)
                ...
                patch[patch==self.no_data]=med
                #print(med)
                for i in range(len(self.unique_values)): patch[patch==self.unique_values[i]]=i 
                patch=patch.astype(np.int16)
            if self.one_hot and not cancel_one_hot:
                try:
                    med=int(stats.mode(patch[patch!=self.no_data],axis=None)[0][0])  ###replace no_data by median value
                ...
                #patch[patch==self.no_data]=np.mean(patch) ###continuous case replace no_data by mean value
                patch = patch[np.newaxis]
...
class PatchExtractor(object):
    ...
    def __getitem__(self, item, cancel_one_hot=False):
        ...
        """
        return np.stack([r.__getitem__(item, cancel_one_hot) for r in self.rasters],axis=-1)
